...
def get_genos_aloci(ind):
    ...
    second_alleles = sim_genos[(num_sim_genos/2):int(num_sim_genos/2)+num_aloci]
    for zipped_geno in zip(first_alleles, second_alleles):
        genos.append(trans_geno(zipped_geno))     
    return(genos)

def get_mag_locus(l_idx, num_loci, magdiff):
    '''Get relative magnitude of this locus'''
    sum_effects = 0 
    for j in range(1,num_loci+1):
        sum_effects += np.e**(-magdiff*j)
    this_effect = np.e**(-magdiff*l_idx) / sum_effects
    return(this_effect)

def get_fitness(ind, pop):
    '''Get fitness, using decreasing exponential effects of loci, and fitness
    effects per locus, per life history stage, per subpop.'''
    ...
    for geno_count, geno_item in enumerate(genos_this_stage):
        mag = get_mag_locus(geno_count, num_aloci, magdiff)
        locus_effect = fit_dict[this_sp_name][stage][geno_count][geno_item]
        fitness += mag*locus_effect # get magnitude of effect X locus fitness effect, add to running sum
    return(fitness)

def assign_fitness_all(pop):
    '''Assign fitness, using decreasing exponential effects of loci, and fitness
    effects per locus, per life history stage, per subpop, for all individuals.'''
...
def vbgrow_step_farm(age, sd_growth_farm):
    '''Von Bertalanffy growth based on age, with log normal error & effect of climate deviation;
    to be used in pop_vbgrowth_step'''
    size = Linf*(1-np.exp(-vb_K*(age-t0)))*np.exp(sd_growth_farm-(sd_growth_farm*sd_growth_farm)/2)
    return(size)

def vbgrow_step_wild(age, sd_growth_wild):
    '''Von Bertalanffy growth based on age, with log normal error & effect of climate deviation;
    to be used in pop_vbgrowth_step'''
    size = Linf*(1-np.exp(-vb_K*(age-t0)))*np.exp(sd_growth_wild*clim_dev[year_counter]-(sd_growth_wild*sd_growth_wild)/2)
    return(size)

def pop_vbgrowth_step(farm_boo, farm_idx, subPopIndeces, sd_growth_wild, sd_growth_farm):
    '''Von bert growth applied to individuals of each subPop'''  
    if farm_boo == False: # if not a farm year
        for this_subPop in subPopIndeces:
            pop.setIndInfo(values=list(np.apply_along_axis(vbgrow_step_wild, 0, 
                                                           np.array(pop.indInfo(field='age', subPop=this_subPop)),
                                                           sd_growth_wild)), field='size', subPop=this_subPop)        
    else: # if a farm year
        this_subPop = farm_idx
        pop.setIndInfo(values=list(np.apply_along_axis(vbgrow_step_farm, 0, 
                                                       np.array(pop.indInfo(field='age', subPop=this_subPop)),
                                                       sd_growth_farm)), field='size', subPop=this_subPop)
        remaining_subPops = list(subPopIndeces)
        remaining_subPops.remove(farm_idx)
        for this_subPop in remaining_subPops:
            pop.setIndInfo(values=list(np.apply_along_axis(vbgrow_step_wild, 0, 
                                                           np.array(pop.indInfo(field='age', subPop=this_subPop)),
                                                           sd_growth_wild)), field='size', subPop=this_subPop)
            
def wo_farm_recruitment(pop):
    '''Provide next time step wild subpopulation sizes before farm phase, exp growth'''
...
def w_farm_recruitment(pop):
    '''Provide next time step wild subpopulation sizes in farm phase'''
...
def get_mort_fitfactor(ind_fitness):
    ...
    fitfactor = (((ind_fitness- 0) * (2*var_in_mort)) / 1) + (1-var_in_mort)
    return(fitfactor)
...
def mortality(pop, lar_annual_M, juv_annual_M, adult_annual_M, farm_reduced_mort):
    ...
    for sp_idx, sp_name in enumerate(pop.subPopNames()):
        ...
        for ind in pop.individuals(sp_name): 
            draw = random.random()
            fitfactor = get_mort_fitfactor(ind_fitness=ind.fitness)
            if ind.age < settle_age and draw > np.exp((-lar_annual_M/12)*fitfactor*farm_wild_factor):
                ind.kill = 1
            elif settle_age <= ind.age < repro_age and draw > np.exp((-juv_annual_M/12)*fitfactor*farm_wild_factor): 
                ind.kill = 1
            elif repro_age <= ind.age < max_age and draw > np.exp((-adult_annual_M/12)*fitfactor*farm_wild_factor):
                ind.kill = 1
            elif ind.age >= max_age:
                ind.kill = 1               
        pop.removeSubPops([(sp_idx,vsp_n2i['to_kill'])]) # kill individuals

def mark_broodstock(pop, source_index, source_name, mat_vsp_name, num_broodstock, eq_sex):
    ...
    # CHECK
    for ind in pop.individuals(source_name): 
        if ind.broodstock == 1:
            print("Ruh roh! Broodstock already marked.")
    
    # store female and male idx that are in source pop & sexually mature
    mature_F_idx = []
    mature_M_idx = []
    for index in range(pop.subPopBegin(source_index), pop.subPopEnd(source_index)):
...
def get_w_farm_wld_sp_idcs(num_wild_pops, source_index):
    '''Get list of wild pop subpop indeces, which
    depends on the broodstock source_index. '''
    wld_sp_idcs = list(range(num_wild_pops+1))
    del wld_sp_idcs[source_index+1]
    return(wld_sp_idcs)

def get_sp_names(source_index, farm_boo, wild_names):
    '''Get list of subpop names, which
    depends on whether the farm is included, 
    and the broodstock source_index.'''
...
def store_afs(pop, rep, year_counter, farm_boo, farm_idx, afs_dict, loci_to_track):
    ...
    if farm_boo == True:
        for herein_index, sp_index in enumerate(w_farm_wld_sp_idcs):  
            for locus in loci_to_track:
                for allele in [0,1]:
                    afs_dict[rep][year_counter][herein_index][locus][allele] = \
                    pop.vars()['subPop'][sp_index]['alleleFreq'][locus][allele]        
        for locus in loci_to_track:            
            for allele in [0,1]:
                afs_dict[rep][year_counter]['farm'][locus][allele] = \
                pop.vars()['subPop'][farm_idx]['alleleFreq'][locus][allele] 
    else:
        for sp_index in range(num_wild_pops):
            for locus in loci_to_track:
                for allele in [0,1]:
                    afs_dict[rep][year_counter][sp_index][locus][allele] = \
                    pop.vars()['subPop'][sp_index]['alleleFreq'][locus][allele]
                    
# store genotype frequencies                    
def store_gfs(pop, rep, year_counter, farm_boo, farm_idx, gfs_dict, loci_to_track):
    '''Stores genotype frequences into dictionary'''
    if farm_boo == False: # if not farm year
        for index in range(num_wild_pops):
            for locus in loci_to_track:
                freq_het = 0
                for geno in [(0,1),(1,0)]:
                    freq_het += pop.vars()['subPop'][index]['genoFreq'][locus][geno]
                gfs_dict[rep][year_counter][index][locus]['00'] = \
                pop.vars()['subPop'][index]['genoFreq'][locus][(0,0)]
                gfs_dict[rep][year_counter][index][locus]['11'] = \
                pop.vars()['subPop'][index]['genoFreq'][locus][(1,1)]
                gfs_dict[rep][year_counter][index][locus]['01'] = freq_het
    else: # if farm year
        for index, item in enumerate(w_farm_wld_sp_idcs): # wild subpops
            for locus in loci_to_track:
                freq_het = 0
                for geno in [(0,1),(1,0)]:
                    freq_het += pop.vars()['subPop'][item]['genoFreq'][locus][geno]
                gfs_dict[rep][year_counter][index][locus]['00'] = \
                pop.vars()['subPop'][item]['genoFreq'][locus][(0,0)]
                gfs_dict[rep][year_counter][index][locus]['11'] = \
                pop.vars()['subPop'][item]['genoFreq'][locus][(1,1)]
                gfs_dict[rep][year_counter][index][locus]['01'] = freq_het     
        for locus in loci_to_track: # farm
                freq_het = 0
                for geno in [(0,1),(1,0)]:
                    freq_het += pop.vars()['subPop'][farm_idx]['genoFreq'][locus][geno]
                gfs_dict[rep][year_counter]['farm'][locus]['00'] = \
                pop.vars()['subPop'][farm_idx]['genoFreq'][locus][(0,0)]
                gfs_dict[rep][year_counter]['farm'][locus]['11'] = \
                pop.vars()['subPop'][farm_idx]['genoFreq'][locus][(1,1)]
                gfs_dict[rep][year_counter]['farm'][locus]['01'] = freq_het
...
def store_fst(pop, pair_rvar_dict, rep, year_counter, pop_pairs):
...
def store_fst_adaploci(pop, pair_rvar_dict, rep, year_counter, pop_pairs):
...
def store_fst_neuloci(pop, pair_rvar_dict, rep, year_counter, pop_pairs):
...
def store_rvars(pop, farm_boo, farm_idx, rep, year_counter, afs_dict, gfs_dict, loci_to_track, 
    ...
    store_fst(pop=pop, pair_rvar_dict=pair_rvar_dict, rep=rep, year_counter=year_counter, pop_pairs=pop_pairs)
    store_fst_adaploci(pop=pop, pair_rvar_dict=pair_rvar_dict, rep=rep, 
                       year_counter=year_counter, pop_pairs=pop_pairs)
    ...
    store_gfs(pop=pop, rep=rep, year_counter=year_counter, farm_boo=farm_boo, farm_idx=farm_idx, 
              gfs_dict=gfs_dict, loci_to_track=loci_to_track)
...
# from command line
batch_ID = sys.argv[1] # input batch ID as first arg in command line
reps = int(sys.argv[2]) # number of reps within this iteration of simulations
srep = int(sys.argv[3]) # "super rep," so can combine across simulations from diff cores

# input params 
pre_farm_years = 50 # num of gens first (of three) section of the model
farm_years = 50 # num of gens second section of the model
post_farm_years = 50 # num of gens third section of the model
wild_N_init = 2000 # 5000
rec_a = 200 # 500
farm_nonB_recruit_factor = 0.5 # recruitment due to nonB adults on farm, as prop of wild recruitment
gamEsc_rec = 5 # 10 # monthly
seed_batch_size = rec_a*3 # reca*4 number of seed planted on farm at once
num_aloci_stage = 5 # number of adaptive loci
num_nloci = 10 # number of neutral loci
track_AFs = True # needs to be True rn, or else R script will crash
track_GFs = False
make_report_all_inds = True
make_report_adults = True

numWildOffspring_par = 1
numFarmOffspring_par = rec_a
numGameteEscapeOffspring_par = 1
...
num_wild_pops = 3
sd_recruit = 0.01
sd_seed = 0.01 # standard deviation of seed size 
source_index = 0 # broodstock source index
local_wild_idx = 0 # index of subpop that farm will escape to 
num_broodstock = 2 # number of broodstock to be collected
eq_sex = False # boolean; T =  eq females & males in bstock; F = random sex ratio, at least one male & female
adult_annual_M = 0.0226 # adult mortality rate per year
var_in_mort = 0.05 # 5% above and 5% below = 10% total variation in mortality due to selection
max_age = 20 # max age in years
max_harvest_age = 5 # harvest all animals once they reach this age
harvest_age = 3 # minimum harvest age in years
harvest_rate = 1/12 # proportion of animals to harvest on farm at once, per month
settle_age = 1/12
repro_age = 2 # age at maturity in years
ageX = repro_age # age to start enumerating individuals (for Ne), for now age at maturity

# monthly stage based escape rates
L_escape_rate = 0.5/12
J_escape_rate = 0/12
A_escape_rate = 0/12

# stage based migration rates
wild_mig_rate_L = 0.01/12 
wild_mig_rate_J = 0
wild_mig_rate_A = 0

prob_repro_by_month = {"Jan":0, "Feb":0, "Mar":0, 
                       "Apr":1, "May": 0, "Jun": 0, 
                       "Jul": 0, "Aug":0, "Sep":0, 
                       "Oct":0, "Nov":0, "Dec":0}
prob_prodseed_by_month = {"Jan":0, "Feb":0, "Mar":0, 
                          "Apr":1, "May": 0, "Jun": 0, 
                          "Jul": 0, "Aug":0, "Sep":0, 
                          "Oct":0, "Nov":0, "Dec":0}
prob_L_G_escape_by_month = {"Jan":0, "Feb":0, "Mar":0, 
                        "Apr":1, "May": 0, "Jun": 0, 
                        "Jul": 0, "Aug":0, "Sep":0, 
                        "Oct":0, "Nov":0, "Dec":0}
prob_J_A_escape_by_month = {"Jan":1, "Feb":1, "Mar":1, 
                        "Apr":1, "May": 1, "Jun": 1, 
                        "Jul": 1, "Aug":1, "Sep":1, 
                        "Oct":1, "Nov":1, "Dec":1}

pop_pairs = ['wild1_wild2', 'wild2_wild3', 'wild1_wild3']
wo_farm_sp_idcs = [0,1,2] # subpop indeces without the farm
w_farm_sp_idcs = [0,1,2,3] # subpop indeces with the farm
start_freq_all = [0.5,0.5] # set starting allele frequencies for neutral loci
gr_error_stdev = 0.05 # st dev around error in logistic growth fn for subpops
Linf = 101.32 # L infinity for Von Bertalanffy growth
vb_K = 0.211 # growth coefficient K for Von Bertalanffy growth
t0 = -1.5 # time zero for Von Bertlanaffy growth
sd_growth_wild = 0.01
sd_growth_farm = 0.005
path_to_Rscript = "/mnt/hgfs/SHARED_FOLDER/GRs_local/version3/plot_rvars_over_time.R"
magdiff = 0.15 # Castellani et al had it 0.15
stages = ['L', 'J', 'A']
num_aloci = num_aloci_stage*len(stages) # number adaptive loci across stages
deg_add = {'L': 0, 'J': 1, 'A': 1}
farm_reduced_mort = 0.5 # mortality on farm will be less than in wild by this factor
xyrs_new_bstock = 5 # get fresh broodstock every x years
...
N = num_nloci + num_aloci # total number of loci
loci_to_track = range(N)
bad_start_AF = 1-good_start_AF
wild_names = ["".join(['wild',str(i+1)]) for i in range(num_wild_pops)]
age_vsp_idcs = list(range(len(stages)))
source_name = wild_names[source_index]
farm_idx = source_index+1
wo_farm_sp_names = get_sp_names(source_index=source_index, farm_boo=False, wild_names=wild_names)
wo_farm_sp_indcs = list(range(num_wild_pops))
w_farm_sp_names = get_sp_names(source_index=source_index, farm_boo=True, wild_names=wild_names)
w_farm_wld_sp_idcs = get_w_farm_wld_sp_idcs(source_index=source_index, num_wild_pops=3) # wild subpop indeces with farm
w_farm_sps_to_clone = get_sps_to_clone(sp_idcs=w_farm_sp_idcs, age_vsp_idcs=age_vsp_idcs)
juv_annual_M = adult_annual_M*2 # juvenile mortality rate per year
lar_annual_M = 0.5 #adult_annual_M*10 # larval mortality rate per year
batch_ID = "_".join([str(batch_ID),str(srep)])
...
print("Initializing response variable storage objects and logs")

logfilename = "_".join(["batch",batch_ID,"log.txt"]) # name log file with batch id
if os.path.isfile(logfilename): # if exists, remove old to prevent appending
    os.remove(logfilename)

# write final time stamp to log file
with open(logfilename, "a") as logfile:            
...
bstock_report = open("".join(["batch_", batch_ID, "_bstock_report.txt"]), "w")
bstock_report.write("Rep\tYear\tIndID_survivedAgeX\n")
if make_report_adults:
    track_for_Ne = open("".join(["batch_", batch_ID, "_trackingForNE.txt"]), "w")
...
if make_report_all_inds:
    life_hist_report = open("".join(["batch_", batch_ID, "_life_hist_report.txt"]), "w")
    life_hist_report.write("Rep\tYear\tMonth\tSubpop\tIndID\tCohortYear\tAge\tMother_id\tFather_id\tmeanParentAge\n")

# for rvars that occur per subpop, per year
pop_rvar_dict = {}
...
for rep in range(reps):
    ...
    for year in range(total_years):
        gfs_dict[rep][year] = {}
        for wild_sp in range(num_wild_pops):
            gfs_dict[rep][year][wild_sp] = {}
            for l_idx in range(N):
                gfs_dict[rep][year][wild_sp][l_idx] = {}
        if total_years_farm_boo[year] == True: # it's a farm year
            gfs_dict[rep][year]['farm'] = {}
            for l_idx in range(N):
                gfs_dict[rep][year]['farm'][l_idx] = {}

# for rvars that occur per pair of subpops, per year
pop_pair_rvar_dict = {}
for rep in range(reps):
...
# for rvars that occur per subpop, across time
temp_rvar_dict = {}
for rep in range(reps):
    temp_rvar_dict[rep] = {}
...
# for storing harvest info
harvest_dict = {}
for rep in range(reps):
    harvest_dict[rep] = {}
    for year in range(total_years):
        harvest_dict[rep][year] = [] # per rep per year, number inds harvested
...
for rep in range(reps):
    ...
    # assign climate deviation series per model rep 
    ...
    pop = sim.Population(size=[wild_N_init]*num_wild_pops, 
                        ...
                        infoFields=['fitness', 'migrate_to', 'ind_id','age',
                                    'meanParentAge', 'size', 'broodstock', 'kill',
    ...
    for ind in pop.individuals():
        ...
        ind.size = Linf*(1-np.exp(-vb_K*(ind.age-t0))) 
    sim.initSex(pop) 
    ...
    sim.initGenotype(pop, loci=range(num_aloci,N), freq=start_freq_all) # can set to be different - for now, same
    ...
    # desginated subpops to clone and mate
    ...
    for year in np.arange(pre_farm_years):
        ...
        for month in prob_repro_by_month:
            ...
            if make_report_all_inds:
                track_all_inds(sp_names=wo_farm_sp_names)
            ...
            pop_vbgrowth_step(farm_boo=False, farm_idx=farm_idx, # grow
                              subPopIndeces=wo_farm_sp_idcs, 
                              sd_growth_wild=sd_growth_wild, sd_growth_farm=sd_growth_farm)
            ...
            if random.random() < prob_repro_by_month[month]:
                pop.evolve(preOps=[sim.PySelector(func=get_fitness),
                                   sim.Stat(effectiveSize=range(num_aloci,N), 
                                            subPops=wo_farm_sp_idcs, vars='Ne_demo_base_sp')],
                        ...
                        postOps=[sim.Stat(alleleFreq=range(N), subPops=wo_farm_sp_idcs, vars=['alleleFreq','alleleFreq_sp']),
                             sim.Stat(genoFreq=range(N), subPops=wo_farm_sp_idcs, vars=['genoFreq','genoFreq_sp']),
                             sim.Stat(structure=range(N), subPops=[0,0], vars=['F_is']),
                             sim.Stat(structure=range(N), subPops=[0,1], suffix='_wild1_wild2', vars=['F_st']),
                             sim.Stat(structure=range(N), subPops=[1,2], suffix='_wild2_wild3', vars=['F_st']),
                             sim.Stat(structure=range(N), subPops=[0,2], suffix='_wild1_wild3', vars=['F_st']),
                             ...
                             sim.Stat(structure=range(num_aloci,N), subPops=[0,1], 
                                      suffix='_nL_wild1_wild2', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci,N), subPops=[1,2], 
                                      suffix='_nL_wild2_wild3', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci,N), subPops=[0,2], 
                ...
                # debugging numoffspring 20200501
                # get_parents(subpop_indeces = wo_farm_sp_idcs)
                
            if month == "Dec": # pre farm
                
                # store response variables
                store_rvars(pop=pop, farm_boo=False, farm_idx=farm_idx, rep=rep, year_counter=year_counter, 
                            afs_dict=afs_dict, gfs_dict=gfs_dict, loci_to_track=loci_to_track, rvar_dict=pop_rvar_dict, 
                            pair_rvar_dict=pop_pair_rvar_dict, wild_subPopIndeces=wo_farm_sp_indcs,
        ...
        if year_counter % yr_interval_log == 0:
            ...
            write_to_log(logfilename=logfilename, pop=pop, farm_idx=farm_idx, farm_boo=False, farm_phase="pre-farm")
        year_counter += 1
        
        if year == pre_farm_years-1: # in last year, save population as object to get rvar temporal fst
            pop.save("".join(["batch_", batch_ID, "_t0.pop"]))
    ...
    w_farm_sps_to_mate_wild = get_sps_to_mate_w_farm(wild_or_farm='wild_and_nonBfarm', farm_idx=farm_idx, wild_idcs=w_farm_wld_sp_idcs)        
    ...
    for year in np.arange(farm_years):
        ...
        for month in prob_repro_by_month:
            ...
            if created_farm == False: # arbitrarily create farm population at the very beginning of farm phase
                mark_broodstock(pop, source_index=source_index, source_name=source_name, 
                                mat_vsp_name='A', num_broodstock=num_broodstock, eq_sex=False)                   
                sim.splitSubPops(pop, subPops=source_index, infoFields='broodstock', names=[source_name, 'farm']) 
                pop.evolve(preOps=[sim.PySelector(func=get_fitness)],
                        matingScheme=sim.HeteroMating([sim.CloneMating(subPops=w_farm_sps_to_clone, weight=-1),
                                                     ...
                                                     subPopSize=seed_production(pop=pop, farm_idx=farm_idx, 
                                                                                ...
                                                                                sd_seed=sd_seed)),
                        postOps=[sim.Stat(alleleFreq=range(N), subPops=w_farm_sp_idcs, vars=['alleleFreq','alleleFreq_sp']),
                             sim.Stat(genoFreq=range(N), subPops=w_farm_sp_idcs, vars=['genoFreq','genoFreq_sp']),
                             sim.Stat(structure=range(N), subPops=[w_farm_wld_sp_idcs[0], w_farm_wld_sp_idcs[1]], 
                                      suffix='_wild1_wild2', vars=['F_st']),
                             sim.Stat(structure=range(N), subPops=[w_farm_wld_sp_idcs[1], w_farm_wld_sp_idcs[2]], 
                                      suffix='_wild2_wild3', vars=['F_st']),
                             sim.Stat(structure=range(N), subPops=[w_farm_wld_sp_idcs[0], w_farm_wld_sp_idcs[2]], 
                                      suffix='_wild1_wild3', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci), subPops=[0,1], suffix='_aL_wild1_wild2', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci), subPops=[1,2], suffix='_aL_wild2_wild3', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci), subPops=[0,2], suffix='_aL_wild1_wild3', vars=['F_st']),
            ...
            if random.random() < prob_L_G_escape_by_month[month]:
                ...
                for ind in pop.individuals(farm_idx):
                    if ind.age >= repro_age and ind.broodstock == 0:
                        ...
                        if ind.sex() == 1:
                            ...
                            farm_female = True
                    elif ind.age >= repro_age and ind.broodstock == 1: # bc bstock were marked nonB_adult = 1 prevsly when wild
                        ind.nonB_adult = 0
                if farm_male and farm_female:
                    nonB_farm_adults = True
                ...
                if nonB_farm_adults and nonB_wild1_adults:
                    ...
                    pop.evolve(preOps=[sim.PySelector(func=get_fitness_hybrid)],
                                   matingScheme=sim.HeteroMating([sim.CloneMating(weight=-1),
                                                        ...
                                                        sim.OffspringGenerator(ops=[sim.MendelianGenoTransmitter(), 
                                                                              ...
                                                                              numOffspring=numGameteEscapeOffspring_par),
                                                                                   subPops=hybrid_index)],
                                                                 subPopSize=scaleGameteEscape(pop)), gen=1)
                    ...
                    for sp_index in range(len(wild_names)+2):
                        if sp_index == hybrid_index:
                            for ind in pop.individuals(sp_index):
             ...
             # track individual information for life history table
            if make_report_all_inds:
                track_all_inds(sp_names=w_farm_sp_names)        
            ...
            pop_vbgrowth_step(farm_boo=True, farm_idx=farm_idx, # grow
                              subPopIndeces=w_farm_sp_idcs, 
                              sd_growth_wild=sd_growth_wild, sd_growth_farm=sd_growth_farm)
            pop.setIndInfo(values=list(np.array(pop.indInfo(field='age'))+1/12), field='age') # age
            assign_fitness_all(pop) # for selective mortality
            mortality(pop=pop, lar_annual_M=lar_annual_M, juv_annual_M=juv_annual_M, 
            ...
            if random.random() < prob_prodseed_by_month[month]:
                ...
                if min1male == False or min1female == False or year_counter % xyrs_new_bstock == 0:
                    ...
                    pop.removeSubPops([(farm_idx,vsp_n2i['bstock'])]) # remove all bstock
                    pop.setIndInfo(values=0, field='broodstock') # make sure no animals marked bstock, including recently migrated
                    # collect new bstock, by mark and collect
                    mark_broodstock(pop=pop, source_index=source_index, source_name=source_name, 
                                    mat_vsp_name='A', num_broodstock=num_broodstock, eq_sex=False)
                ...
                pop.evolve(preOps=[sim.PySelector(func=get_fitness)],
                        ...
                        postOps=[sim.Stat(alleleFreq=range(N), subPops=w_farm_sp_idcs, vars=['alleleFreq','alleleFreq_sp']),
                             sim.Stat(genoFreq=range(N), subPops=w_farm_sp_idcs, vars=['genoFreq','genoFreq_sp']),
                             sim.Stat(structure=range(N), subPops=w_farm_wld_sp_idcs[0], vars=['F_is'], 
                                      suffix= "_wild1"),
                             sim.Stat(structure=range(N), subPops=w_farm_wld_sp_idcs[1], vars=['F_is'], 
                                      suffix= "_wild2"),
                             sim.Stat(structure=range(N), subPops=w_farm_wld_sp_idcs[2], vars=['F_is'], 
                                      suffix= "_wild3"),                                  
                             sim.Stat(structure=range(N), subPops=[w_farm_wld_sp_idcs[0], w_farm_wld_sp_idcs[1]], 
                                      suffix='_wild1_wild2', vars=['F_st']),
                             sim.Stat(structure=range(N), subPops=[w_farm_wld_sp_idcs[1], w_farm_wld_sp_idcs[2]], 
                             ...
                             sim.Stat(structure=range(N), subPops=[w_farm_wld_sp_idcs[0], w_farm_wld_sp_idcs[2]], 
                                      suffix='_wild1_wild3', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci,N), subPops=[w_farm_wld_sp_idcs[0], w_farm_wld_sp_idcs[1]], 
                                      suffix='_nL_wild1_wild2', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci,N), subPops=[w_farm_wld_sp_idcs[1], w_farm_wld_sp_idcs[2]], 
                                      suffix='_nL_wild2_wild3', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci,N), subPops=[w_farm_wld_sp_idcs[0], w_farm_wld_sp_idcs[2]], 
                                      suffix='_nL_wild1_wild3', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci), subPops=[w_farm_wld_sp_idcs[0], w_farm_wld_sp_idcs[1]], 
                                      suffix='_aL_wild1_wild2', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci), subPops=[w_farm_wld_sp_idcs[1], w_farm_wld_sp_idcs[2]], 
                                      suffix='_aL_wild2_wild3', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci), subPops=[w_farm_wld_sp_idcs[0], w_farm_wld_sp_idcs[2]], 
                                      suffix='_aL_wild1_wild3', vars=['F_st']),
                             sim.Stat(popSize=True, subPops=w_farm_sp_idcs)],
                        gen=1)     
            ...
            if month == "Dec": # during farm
                ...
                store_rvars(pop=pop, farm_boo=True, farm_idx=farm_idx, rep=rep, 
                            year_counter=year_counter, afs_dict=afs_dict, gfs_dict=gfs_dict, 
                            loci_to_track=loci_to_track, rvar_dict=pop_rvar_dict, 
                            pair_rvar_dict=pop_pair_rvar_dict, wild_subPopIndeces=wo_farm_sp_indcs,
        ...
        for month in prob_repro_by_month:
            ...
            if random.random() < prob_repro_by_month[month]:
                pop.evolve(preOps=[sim.PySelector(func=get_fitness)],
                        ...
                        postOps=[sim.Stat(alleleFreq=range(N), subPops=wo_farm_sp_idcs, vars=['alleleFreq','alleleFreq_sp']),
                             sim.Stat(genoFreq=range(N), subPops=wo_farm_sp_idcs, vars=['genoFreq','genoFreq_sp']),    
                             sim.Stat(structure=range(N), subPops=0, vars=['F_is'], suffix= "_wild1"),
                             sim.Stat(structure=range(N), subPops=1, vars=['F_is'], suffix= "_wild2"),
                             sim.Stat(structure=range(N), subPops=2, vars=['F_is'], suffix= "_wild3"), 
                             sim.Stat(structure=range(N), subPops=[0,1], suffix='_wild1_wild2', vars=['F_st']),
                             sim.Stat(structure=range(N), subPops=[1,2], suffix='_wild2_wild3', vars=['F_st']),
                             sim.Stat(structure=range(N), subPops=[0,2], suffix='_wild1_wild3', vars=['F_st']),
                             sim.Stat(structure=range(num_aloci), subPops=[0,1], 
    ...
    pop_t0 = sim.loadPopulation("".join(["batch_", batch_ID, '_t0.pop']))
    ...
    for i in range(len(t1_names)):
        sim.stat(pop, structure=range(N), subPops=[i,i+len(t1_names)], 
...
# for pop_rvar_dict 
pop_rvar_outfile = open("".join(["batch_", batch_ID, "_pop_rvars.txt"]), "w")
        ...
        for sp in pop_rvar_dict[rep][year]:
            for rvar in pop_rvar_dict[rep][year][sp]:
                if sp !="farm":
                    ...
                    sp_name = sp
                pop_rvar_outfile.write("\t".join([str(srep), str(rep), str(year), sp_name,  
                                                  rvar, str(pop_rvar_dict[rep][year][sp][rvar])]))
                pop_rvar_outfile.write("\n")
pop_rvar_outfile.close()

# for harvest
harvest_outfile = open("".join(["batch_", batch_ID, "_harvest.txt"]), "w")
harvest_outfile.write("Srep\tRep\tYear\tIndsHarvested\n")
for rep in harvest_dict:
    for year in harvest_dict[rep]:
        harvest_outfile.write("\t".join([str(srep), str(rep), str(year), str(harvest_dict[rep][year])]))
        harvest_outfile.write("\n")
harvest_outfile.close()

if track_AFs == True:
    afs_outfile = open("".join(["batch_", batch_ID, "_AFs.txt"]), "w")
        ...
        for year in afs_dict[rep]:
            for sp in afs_dict[rep][year]:
                if sp !="farm":
                    ...
                    sp_name = sp
                for l_idx in afs_dict[rep][year][sp]:
                    if l_idx in range(num_aloci):
                        adaptive_locus = True
                        for allele in [0,1]:
                            stage = stages[l_idx//num_aloci_stage]
                            within_stage_l_idx = l_idx % num_aloci_stage
                            if fit_dict[sp_name][stage][within_stage_l_idx][str(allele)] == 1:
                                ...
                                advantage = False
                            afs_outfile.write("\t".join([str(srep), str(rep), str(year), sp_name, 
                                                         str(l_idx), str(allele), str(adaptive_locus),
                                                         str(advantage), str(afs_dict[rep][year][sp][l_idx][allele])]))
                            afs_outfile.write("\n")    
                    else:
                        adaptive_locus = False
                        advantage = "NA"
                        for allele in [0,1]:
                            afs_outfile.write("\t".join([str(srep), str(rep), str(year), sp_name, str(l_idx), 
                                                         str(allele), str(adaptive_locus), str(advantage), 
                                                         str(afs_dict[rep][year][sp][l_idx][allele])]))
                            afs_outfile.write("\n")  
    afs_outfile.close()
    
if track_GFs == True:
    gfs_outfile = open("".join(["batch_", batch_ID, "_GFs.txt"]), "w")
    gfs_outfile.write("Srep\tRep\tYear\tSubpop\tLocus_index\tGenotype\tAdaptive\tGenotypeFrequency\n")
    for rep in gfs_dict:
        for year in gfs_dict[rep]:
            for sp in gfs_dict[rep][year]:
                if sp !="farm":
                    sp_name = "".join(["wild", str(sp+1)])
                else:
                    sp_name = sp
                for locus in gfs_dict[rep][year][sp]:
                    if locus in range(num_aloci):
                        adaptive_locus = True
                    else:
                        adaptive_locus = False   
                    for geno in ['00', '01', '11']:
                        gfs_outfile.write("\t".join([str(srep),  str(rep), str(year), sp_name,
                                                     str(locus), geno, str(adaptive_locus),
                                                     str(gfs_dict[rep][year][sp][locus][geno])]))
                        gfs_outfile.write("\n")                                                       
    gfs_outfile.close()
    
# for pop_pair_rvar_dict
pop_pair_rvar_outfile = open("".join(["batch_", batch_ID, "_pop_pair_rvars.txt"]), "w")
    ...
    for year in pop_pair_rvar_dict[rep]:
        for pop_pair in pop_pair_rvar_dict[rep][year]:
            for rvar in pop_pair_rvar_dict[rep][year][pop_pair]:
                pop_pair_rvar_outfile.write("\t".join([str(srep), str(rep), str(year), pop_pair, rvar, 
...
temp_rvar_outfile = open("".join(["batch_", batch_ID, "_temp_rvars.txt"]), "w")
    ...
    for sp in temp_rvar_dict[rep]:
        for rvar in temp_rvar_dict[rep][sp]:
            if sp !="farm":
                ...
                sp_name = sp
            temp_rvar_outfile.write("\t".join([str(srep), str(rep), sp_name, rvar, str(temp_rvar_dict[rep][sp][rvar])]))
            temp_rvar_outfile.write("\n")           
temp_rvar_outfile.close()

# make plots in R
R_plotting_call = "".join(["Rscript ", path_to_Rscript, " ", batch_ID, " ", 
                           str(pre_farm_years), " ", str(pre_farm_years + farm_years)])
subprocess.call([R_plotting_call], shell=True)
...
print("Done! Run time:", time.time() - startTime)
